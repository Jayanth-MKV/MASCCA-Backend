...
import { updatePrivateDto, updatePubDto, UpdateTestDto } from './dto/update-test.dto';
...
export class TestController {
  ...
  @Get('all/search')
  @UseGuards(StudentAuthGuard)
  async findAllSearch(@Query('search') searchQuery: string) {
    if (!searchQuery) {
      return [];
    }

    return await this.testService.searchTests(searchQuery);
  }
  ...
  @Patch('private/:id')
  @UseGuards(InstructorAuthGuard)
  makePrivateTest(@Param('id') id: string, @Body() updatePrivateDto: updatePrivateDto) {
    return this.testService.MakePrivate(id, updatePrivateDto);
  }
